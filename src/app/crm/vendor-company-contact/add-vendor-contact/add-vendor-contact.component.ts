...
export class AddVendorContactComponent implements OnInit {
  ...
  addcompany(emailId, countryid, primaryphone, companyname) {
    ...
    if (emailId !== "" && countryid !== "" && primaryphone !== "" && companyname !== "") {
      ...
      this.loading = false;
      this.contactError = 'Fill required Fields';
    }
  ...
  checkClientName(){
    ...
    console.log(this.cusData.companyName);
  ...
  //  check company contact email added by suresh -12-04-2019 start

  checkCompanyContactEmail(){
    this.emailError = "";
    this.checkData.companyId = this.cusData.companyId;
    this.checkData.emailId = this.cusData.emailId;
    this.checkData.companyType = "vendor";  
    this.service.checkCompanyContactEmail(this.checkData)
    .subscribe(res => {
  ...
  //  check company contact email added by suresh -12-04-2019 end

   // check company email added by suresh -12-04-2014 start 
   checkCompanyEmail(){
    this.vendorEmailError = "";
    this.vendorCompanyChecking.companyType = "vendor";  
    this.vendorCompanyChecking.emailId = this.cusData.emailId;  
    this.service.checkCompanyEmail(this.vendorCompanyChecking)
    .subscribe(res => {
      this.checkingsompany = res.json();
      if(this.checkingsompany.statusCode.code === "200"){
        this.vendorEmailError = this.checkingsompany.data;
      }
      else if(this.checkingsompany.statusCode.code === "404"){
        this.vendorEmailError = "";
      }
      else{
        this.vendorEmailError = this.checkingsompany.errorMessages;
      }
    })
  }
  // check company email added by suresh -12-04-2014 start
