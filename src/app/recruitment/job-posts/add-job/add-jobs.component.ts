...
export class AddJobsComponent implements OnInit {
  ...
  constructor(private http: Http, private router: Router, public auth: AuthenticationService, public service: UserService, public pattern: PatternsService,
    ...
    this.jobcodeLength = this.pattern.clientJobCodelength;
    this.jobTitleLength = this.pattern.jobtitlelength;
    this.minlength = this.pattern.minName;
    this.maxLength = this.pattern.maxName;
    this.internalNoteLength = this.pattern.internalNotesLength;
    this.internalBillratelength = this.pattern.internalBillrateLength;
    this.openingLength = this.pattern.jobopeningslength;
    this.minimumLength = this.pattern.minimumLength;
    this.maximumLength = this.pattern.maximumLength;
    this.companyNameLength = this.pattern.companynameLength;
    this.emailLength = this.pattern.emaillength;
  ...
  dateValidate() {
    this.date1 = new DatePipe('en-US').transform(this.jobData.datePosted, 'yyyy-MM-dd');
    this.date2 = new DatePipe('en-US').transform(this.jobData.targetDate, 'yyyy-MM-dd');
  ...
  closecontPopup(popupContact:NgForm) {
